            ...
            <AnimatePresence mode="wait">
              {selectedPostId && currentPost ? (
                ...
                <div className="space-y-6">
                  ...
                  {/* Guest Pitch compositor trigger */}
                  {!currentUser && activeTab === 'feed' && !searchQuery && (
                    <div 
                      onClick={() => setShowAuthModal(true)}
                      className="backdrop-blur-lg bg-white/70 dark:bg-zinc-900/70 border border-white/80 dark:border-zinc-850/80 p-5 rounded-[32px] shadow-sm flex items-center justify-between cursor-pointer hover:border-[#FF3B30]/50 transition group mb-6 text-left"
                    >
                      <div className="flex items-center gap-3.5">
                        <div className="w-11 h-11 rounded-full bg-[#FF3B30]/5 flex items-center justify-center text-[#FF3B30] group-hover:scale-105 transition duration-300">
                          <MessageSquare className="w-5 h-5 text-[#FF3B30]" />
                        </div>
                        <div>
                          <p className="text-sm font-black text-zinc-800 dark:text-zinc-100 tracking-tight">Share something trending today 🌌</p>
                          <p className="text-xs text-zinc-500">Log in or Sign up to publish opinion, thought, confession or joke.</p>
                        </div>
                      </div>
                      <button className="px-4 py-2 bg-[#FF3B30] hover:bg-red-600 text-white text-xs font-extrabold rounded-full transition shadow-md shadow-red-500/10 hover:scale-105">
                        Compose ✦
                      </button>
                    </div>
                  )}
          ...
          >
            ...
            >
              ...
              <button
                ...
                className="absolute top-4 right-4 text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-zinc-200 bg-white/50 backdrop-blur dark:bg-zinc-800/40 p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 dark:hover:bg-zinc-800 rounded-full border border-zinc-200/45 dark:border-zinc-700/60 shadow-sm flex items-center justify-center cursor-pointer hover:scale-105 transition"
